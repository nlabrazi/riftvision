...
<template>
  <div class="space-y-6">
    ...
    <section data-testid="scoreboard-header"
      class="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 backdrop-blur shadow-2xl">
      <div class="grid grid-cols-3 items-center gap-4">
        <!-- Blue Team Summary -->
        <div class="flex items-center gap-4">
          <div
            class="w-12 h-12 rounded-2xl bg-blue-950 border border-blue-600/40 flex items-center justify-center font-black text-blue-400 text-xl shadow-lg shadow-blue-950/50">
          ...
          <div>
            <div class="flex items-center gap-2">
              ...
              <span
                class="text-xs px-2 py-0.5 rounded-full bg-blue-950/80 border border-blue-800 text-blue-400 font-mono">Order</span>
            </div>
            <div class="text-xs text-slate-400 mt-1 flex items-center gap-3">
              <span class="font-semibold text-slate-300">💰 {{ blueEconomy.totalItemGold.toLocaleString('fr-FR')
                }}g</span>
        ...
        <div class="text-center space-y-1">
          ...
          <!-- Gold Lead Badge -->
          <div class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold shadow-inner" :class="{
            ...
            <span v-else-if="goldDifference < 0">Équipe Rouge +{{ Math.abs(goldDifference).toLocaleString('fr-FR')
              }}g</span>
        ...
        <div class="flex items-center justify-end gap-4 text-right">
          <div>
            <div class="flex items-center justify-end gap-2">
              <span
                class="text-xs px-2 py-0.5 rounded-full bg-rose-950/80 border border-rose-800 text-rose-400 font-mono">Chaos</span>
            ...
            <div class="text-xs text-slate-400 mt-1 flex items-center justify-end gap-3">
              ...
              <span class="font-semibold text-slate-300">💰 {{ redEconomy.totalItemGold.toLocaleString('fr-FR')
                }}g</span>
            </div>
          </div>
          <div
            class="w-12 h-12 rounded-2xl bg-rose-950 border border-rose-600/40 flex items-center justify-center font-black text-rose-400 text-xl shadow-lg shadow-rose-950/50">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <section data-testid="blue-team-column"
        class="bg-slate-900/80 border border-blue-900/40 rounded-2xl p-4 space-y-3 shadow-xl backdrop-blur">
        ...
        <div class="space-y-2.5">
          <div v-for="p in bluePlayers" :key="p.summonerName"
            ...
            <div class="flex items-center gap-3">
              <div
                class="relative w-12 h-12 rounded-xl overflow-hidden border border-slate-700 flex-shrink-0 bg-slate-900">
                <img :src="getChampionIconUrl(p.championName)" :alt="p.championName" class="w-full h-full object-cover"
                  loading="lazy" @error="(e) => ((e.target as HTMLImageElement).src = '/favicon.ico')" />
                <!-- Level Badge -->
                <span
                  class="absolute bottom-0 right-0 bg-slate-950/90 text-cyan-300 font-mono text-[9px] px-1 rounded-tl border-t border-l border-slate-700">
                  {{ p.level }}
                </span>

                <!-- Dead Respawn Overlay -->
                <div v-if="p.isDead"
                  class="absolute inset-0 bg-rose-950/80 backdrop-blur-[1px] flex flex-col items-center justify-center">
                  <span class="text-[9px] font-bold text-rose-300 uppercase">MORT</span>
                  <span class="font-mono text-xs font-black text-rose-100 animate-pulse">{{ Math.ceil(p.respawnTimer)
                    }}s</span>
                </div>
              </div>
              ...
              <div>
                <div class="flex items-center gap-2">
                  ...
                  <span
                    class="text-[10px] font-mono px-1.5 py-0.2 rounded bg-blue-950 border border-blue-800 text-blue-300 font-semibold">
                    {{ p.position || 'FLEX' }}
                  </span>
                </div>
                <div class="text-xs text-slate-400 mt-0.5 flex items-center gap-2">
                  <span class="text-emerald-400 font-semibold">{{ p.scores.kills }}/{{ p.scores.deaths }}/{{
                    p.scores.assists }}</span>
                  <span>•</span>
      ...
      <section data-testid="red-team-column"
        class="bg-slate-900/80 border border-rose-900/40 rounded-2xl p-4 space-y-3 shadow-xl backdrop-blur">
          ...
          <div v-for="p in redPlayers" :key="p.summonerName"
            ...
            <div class="flex items-center gap-3">
              <div
                ...
                <!-- Level Badge -->
                <span
                  class="absolute bottom-0 right-0 bg-slate-950/90 text-rose-300 font-mono text-[9px] px-1 rounded-tl border-t border-l border-slate-700">
                  {{ p.level }}
                </span>
              ...
              <div>
                <div class="flex items-center gap-2">
                  <span class="font-bold text-white text-sm">{{ p.championName }}</span>
                  <span
                    class="text-[10px] font-mono px-1.5 py-0.2 rounded bg-rose-950 border border-rose-800 text-rose-300 font-semibold">
                    {{ p.position || 'FLEX' }}
                  </span>
                </div>
                <div class="text-xs text-slate-400 mt-0.5 flex items-center gap-2">
    ...
    <section data-testid="live-diff-feed"
      class="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 space-y-4 shadow-xl backdrop-blur">
      <div class="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-slate-800">
        ...
        <div class="flex items-center gap-1.5 text-xs">
          <button type="button" @click="feedFilter = 'ALL'" class="px-2.5 py-1 rounded-lg transition"
            :class="feedFilter === 'ALL' ? 'bg-cyan-600 text-white font-bold' : 'bg-slate-800 text-slate-400 hover:text-white'">
            Tous
          </button>
          <button type="button" @click="feedFilter = 'ITEMS'" class="px-2.5 py-1 rounded-lg transition"
            :class="feedFilter === 'ITEMS' ? 'bg-purple-600 text-white font-bold' : 'bg-slate-800 text-slate-400 hover:text-white'">
            Achats d'Items
          </button>
          <button type="button" @click="feedFilter = 'KILLS'" class="px-2.5 py-1 rounded-lg transition"
            :class="feedFilter === 'KILLS' ? 'bg-rose-600 text-white font-bold' : 'bg-slate-800 text-slate-400 hover:text-white'">
            Kills & Morts
          </button>
          <button type="button" @click="feedFilter = 'OBJECTIVES'" class="px-2.5 py-1 rounded-lg transition"
      ...
      <div v-if="filteredEvents.length > 0" class="space-y-2 max-h-72 overflow-y-auto pr-1">
        <div v-for="event in [...filteredEvents].reverse()" :key="event.id"
          class="p-3 rounded-xl bg-slate-950 border text-xs flex items-center justify-between gap-3 transition" :class="{
          ...
          <div class="flex items-center gap-3">
            ...
            <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider" :class="{
              ...
              {{ event.type === 'ITEM_PURCHASE' ? 'Item' : event.type === 'CHAMPION_KILL' ? 'Kill' : event.type ===
                'CHAMPION_DEATH' ? 'Mort' : event.type === 'CHAMPION_RESPAWN' ? 'Respawn' : 'Objectif' }}
            </span>
