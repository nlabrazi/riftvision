  ...
  <div class="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-cyan-500 selection:text-black">
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            <div class="flex items-center gap-2">
              ...
              <span class="text-xs px-2 py-0.5 rounded-full font-mono bg-cyan-950/80 border border-cyan-800 text-cyan-400">v0.1.0</span>
            </div>
    ...
    <main class="flex-1 max-w-7xl w-full mx-auto p-6 space-y-6">
      ...
      <section
        ...
        class="rounded-2xl border p-5 transition-all shadow-xl"
        :class="{
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-xs opacity-80 mt-0.5">
                ...
                <span v-else-if="status.status === 'MOCK'">Jeu de données simulé à 16:45 injecté pour le dev et la preview.</span>
          ...
          <div class="flex items-center gap-3 text-xs">
            ...
            <div class="bg-slate-950/60 rounded-xl px-3 py-2 border border-slate-800 text-center">
              ...
              <div class="font-mono text-sm font-bold text-white">{{ gameData?.gameData?.gameMode || status.gameMode || '—' }}</div>
            </div>
            <div class="bg-slate-950/60 rounded-xl px-3 py-2 border border-slate-800 text-center">
              <div class="text-[10px] uppercase text-slate-400 font-semibold tracking-wider">Joueurs</div>
              <div class="font-mono text-sm font-bold text-white">{{ gameData?.allPlayers?.length || status.playerCount || 0 }}/10</div>
            </div>
            <div class="bg-slate-950/60 rounded-xl px-3 py-2 border border-slate-800 text-center">
              <div class="text-[10px] uppercase text-slate-400 font-semibold tracking-wider">Latence</div>
              <div class="font-mono text-sm font-bold text-cyan-400">{{ status.latencyMs !== undefined ? `${status.latencyMs}ms` : '—' }}</div>
            </div>
          </div>
        </div>

        <div v-if="lastError && status.status === 'DISCONNECTED'" class="mt-3 text-xs text-rose-400/90 font-mono bg-rose-950/40 p-2 rounded-lg border border-rose-900/50">
      ...
      <section class="space-y-4">
        ...
        <div v-if="activeTab === 'players'" class="space-y-4">
          ...
          <div v-else class="text-xs text-slate-500 py-10 text-center bg-slate-900/50 rounded-xl border border-slate-800">
            Aucun joueur actif.
          </div>
        </div>
