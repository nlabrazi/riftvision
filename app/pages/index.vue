...
<template>
  <div
    class="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-cyan-500 selection:text-black">
    <!-- Header -->
    <header class="border-b border-slate-800 bg-slate-900/60 backdrop-blur sticky top-0 z-30 px-6 py-4">
      <div class="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-4">
        <div class="flex items-center gap-3">
          <div
            class="w-10 h-10 rounded-xl bg-gradient-to-tr from-cyan-600 to-indigo-600 flex items-center justify-center font-black text-xl shadow-lg shadow-cyan-950">
            RV
          </div>
          <div>
            <div class="flex items-center gap-2">
              ...
              <span
                class="text-xs px-2 py-0.5 rounded-full font-mono bg-cyan-950/80 border border-cyan-800 text-cyan-400">v0.1.0</span>
            </div>
            <p class="text-xs text-slate-400">Console de diagnostic Riot Live Client API</p>
          </div>
        </div>
        ...
        <div class="flex items-center flex-wrap gap-2">
          <button type="button" data-testid="mock-toggle-button" @click="toggleMockMode"
            class="px-3 py-1.5 rounded-lg text-xs font-semibold transition border flex items-center gap-1.5" :class="status.isMock
              ? 'bg-purple-950/80 border-purple-600 text-purple-200 hover:bg-purple-900'
              : 'bg-slate-800 border-slate-700 text-slate-300 hover:bg-slate-700 hover:text-white'">
            <span class="w-2 h-2 rounded-full"
              :class="status.isMock ? 'bg-purple-400 animate-pulse' : 'bg-slate-500'"></span>
            {{ status.isMock ? 'Mode Mock Actif' : 'Activer Mode Mock' }}
          </button>

          <button type="button" @click="togglePolling"
            class="px-3 py-1.5 rounded-lg text-xs font-semibold transition border flex items-center gap-1.5" :class="isPolling
              ? 'bg-emerald-950/50 border-emerald-700/60 text-emerald-300'
              : 'bg-slate-800 border-slate-700 text-slate-400'">
            <span class="w-2 h-2 rounded-full"
              :class="isPolling ? 'bg-emerald-400 animate-ping' : 'bg-slate-600'"></span>
            {{ isPolling ? 'Polling actif (2s)' : 'Polling en pause' }}
          </button>
    ...
    <main class="flex-1 max-w-7xl w-full mx-auto p-6 space-y-6">
      <!-- Status Banner -->
      <section data-testid="status-banner" class="rounded-2xl border p-5 transition-all shadow-xl" :class="{
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-xs opacity-80 mt-0.5">
                <span v-if="status.status === 'IN_GAME'">Connexion HTTPS locale valide. Polling actif.</span>
                <span v-else-if="status.status === 'MOCK'">Jeu de données simulé à 16:45 injecté pour le dev et la
                  preview.</span>
                <span v-else>Lancez une partie LoL locale (ou activez le Mode Mock ci-dessus).</span>
              </p>
            </div>
          </div>
          ...
          <div class="flex items-center gap-3 text-xs">
            <div class="bg-slate-950/60 rounded-xl px-3 py-2 border border-slate-800 text-center">
              <div class="text-[10px] uppercase text-slate-400 font-semibold tracking-wider">Temps</div>
              <div class="font-mono text-sm font-bold text-white">{{ formattedGameTime }}</div>
            </div>
            <div class="bg-slate-950/60 rounded-xl px-3 py-2 border border-slate-800 text-center">
              <div class="text-[10px] uppercase text-slate-400 font-semibold tracking-wider">Mode</div>
              <div class="font-mono text-sm font-bold text-white">{{ gameData?.gameData?.gameMode || status.gameMode ||
                '—' }}</div>
            </div>
            <div class="bg-slate-950/60 rounded-xl px-3 py-2 border border-slate-800 text-center">
              <div class="text-[10px] uppercase text-slate-400 font-semibold tracking-wider">Joueurs</div>
              <div class="font-mono text-sm font-bold text-white">{{ gameData?.allPlayers?.length || status.playerCount
                || 0 }}/10</div>
            </div>
            <div class="bg-slate-950/60 rounded-xl px-3 py-2 border border-slate-800 text-center">
              <div class="text-[10px] uppercase text-slate-400 font-semibold tracking-wider">Latence</div>
              <div class="font-mono text-sm font-bold text-cyan-400">{{ status.latencyMs !== undefined ?
        ...
        <div v-if="lastError && status.status === 'DISCONNECTED'"
          class="mt-3 text-xs text-rose-400/90 font-mono bg-rose-950/40 p-2 rounded-lg border border-rose-900/50">
      ...
      <section class="space-y-4">
        ...
        <div class="flex border-b border-slate-800 gap-2">
          ...
          <button type="button" data-testid="tab-raw" @click="activeTab = 'raw'"
            class="px-4 py-2 text-xs font-semibold border-b-2 transition"
        ...
        <div v-if="activeTab === 'overview'" class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="bg-slate-900 border border-slate-800 rounded-xl p-4">
            <h3 class="text-xs uppercase tracking-wider text-slate-400 font-semibold mb-3">Joueur Local (Active Player)
            </h3>
          ...
          <!-- Quick Event Log -->
          <div class="bg-slate-900 border border-slate-800 rounded-xl p-4">
            <h3 class="text-xs uppercase tracking-wider text-slate-400 font-semibold mb-3">Derniers Événements Détectés
            </h3>
        ...
        <div v-if="activeTab === 'players'" class="space-y-4">
                  ...
                  <div>
                    <div class="flex items-center gap-2">
                      ...
                      <span class="text-[10px] font-mono px-1.5 py-0.2 rounded bg-slate-800 text-slate-400">{{
                        p.position }}</span>
                      <span v-if="p.isDead"
                        class="text-[10px] font-bold text-rose-400 bg-rose-950/60 px-1.5 py-0.2 rounded">
          ...
          <div v-else
            class="text-xs text-slate-500 py-10 text-center bg-slate-900/50 rounded-xl border border-slate-800">
            Aucun joueur actif.
          </div>
        </div>
        ...
        <div v-if="activeTab === 'events'" class="bg-slate-900 border border-slate-800 rounded-xl p-4">
              ...
              <div class="flex items-center gap-3">
                ...
                <span class="text-slate-300">
                  <template v-if="e.KillerName && e.VictimName">
                    <span class="font-semibold text-emerald-400">{{ e.KillerName }}</span> a tué <span
                      class="font-semibold text-rose-400">{{ e.VictimName }}</span>
                    <span v-if="e.Assisters?.length" class="text-slate-400 text-[11px]"> (Assists: {{
                      e.Assisters.join(', ') }})</span>
                  </template>
                  <template v-else-if="e.EventName === 'FirstBlood'">
                    Premier Sang (First Blood) obtenu par <span class="font-semibold text-emerald-400">{{ e.Recipient
                      }}</span>
                  </template>
          ...
          <div v-else class="text-xs text-slate-500 py-10 text-center">
        ...
        <div v-if="activeTab === 'raw'" class="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <pre class="text-xs font-mono bg-slate-950 p-4 rounded-lg overflow-x-auto text-cyan-300/90 max-h-[500px]">{{
